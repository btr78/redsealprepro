...
def _sniper_cmd_thread():
    import time as _t
    while True:
        try:
            listen_commands(_sniper_tg_ids)
        except Exception:
            pass
        _t.sleep(3)

def listen_commands(telegram_ids):
    global last_upd_id, sniper_paused
    try:
        url  = f"https://api.telegram.org/bot{TELEGRAM_TOKEN}/getUpdates"
        resp = requests.get(url, params={"offset": last_upd_id + 1, "timeout": 3}, timeout=8)
        if not resp.ok:
            return
        for upd in resp.json().get("result", []):
            ...
            if text == "/status":
                ...
                for t in active:
                    entry_ts = datetime.fromisoformat(t["timestamp"]).replace(tzinfo=None)
                    days = (datetime.now() - entry_ts).days
                    act_str += f"\n  {t['ticker']} {t['direction']} day {days+1}"
                reply = (
                    f"*Options Sniper B🤖T v2.0 STATUS*\n"
                    f"Win Rate: {stats['win_rate']:.1f}%\n"
                    f"Wins: {stats['wins']} | Losses: {stats['losses']}\n"
                    f"Avg P&L: {stats['avg_pnl_pct']:+.1f}%\n"
                    f"Total P&L: ${stats['total_pnl_dol']:+.0f}\n"
                    f"Active Trade: {'Yes' + act_str if active else 'None'}"
                )
                send_telegram(reply, [chat_id])
            elif text == "/report":
                send_telegram(weekly_report(), [chat_id])
            elif text == "/active":
                data   = load_trades()
                active = [t for t in data["trades"] if t["status"] == "ACTIVE"]
                if not active:
                    send_telegram("No active trades.", [chat_id])
                else:
                    for t in active:
                        entry_ts = datetime.fromisoformat(t["timestamp"]).replace(tzinfo=None)
                        days = (datetime.now() - entry_ts).days
                        exp_date = datetime.strptime(t["expiration"], "%Y-%m-%d").date()
                        dte = (exp_date - get_today()).days
                        send_telegram(
                            f"*ACTIVE:* {t['ticker']} {t['direction']}\n"
                            f"Contract: {t['contract']}\n"
                            f"Entry: ${t['entry_option_px']:.2f} | Cost: ${t['entry_cost']:.0f}\n"
                            f"DTE: {dte} | Day {days+1} of {MAX_HOLD_DAYS}",
                            [chat_id]
                        )
            elif text == "/pause":
                sniper_paused = True
                send_telegram("\u23f8 Sniper scanning *PAUSED*. Send /resume to restart.", [chat_id])
            elif text == "/resume":
                sniper_paused = False
                send_telegram("\u25b6 Sniper scanning *RESUMED*.", [chat_id])
            elif text == "/watchlist":
                send_telegram(
                    f"*Options Sniper Watchlist ({len(WATCHLIST)})*\n" + ", ".join(WATCHLIST),
                    [chat_id]
                )
            elif text == "/vix":
                try:
                ...
                try:
                    ...
                    for _etf, _name in _SECTORS.items():
                        try:
                            ...
                            _c = _sdf[_etf]["Close"] if isinstance(_sdf.columns, _pd.MultiIndex) else _sdf["Close"]
                    ...
                    for _sn, _se, _sp in _rows:
                        _bar = "\U0001f7e2" if _sp > 0.5 else "\U0001f534" if _sp < -0.5 else "\U0001f7e1"
                        _smsg += f"{_bar} {_sn} ({_se}): {_sp:+.2f}%\n"
                    send_telegram(_smsg, [chat_id])
                except Exception as _e:
                    send_telegram(f"Sectors error: {_e}", [chat_id])
            elif text.startswith("/check "):
                _tk = msg.get("text", "").strip()[7:].upper()
                if not _tk:
...
def main():
    ...
    _sniper_tg_ids.extend(telegram_ids)
    _thr.Thread(target=_sniper_cmd_thread, daemon=True).start()
    log.info("Sniper command listener thread started (polling every 3s)")
    ...
    while True:
        try:
            # listen_commands handled by background thread
            if sniper_paused:
                time.sleep(10)
                continue

            # Update VIX every 30 min
            now_ts = time.time()
